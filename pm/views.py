...
def toggle_project_status(request, pk):
    ...
    return JsonResponse({
        ...
        "completion_date": project.date_of_completion.strftime("%b %d, %Y %H:%M") if project.date_of_completion else None
    })
...
def update_project_engineer(request, pk):
    ...
    try:
        ...
        return JsonResponse({"success": False, "error": "Engineer not found"}, status=400)
...
def download_completion_certificate(request,pk):
    ...
    p.drawString(100, 750, f"Completion Certificate for Project: {project.project_title}")
    p.drawString(100, 730, f"Customer: {project.customer_name.name}")
    p.drawString(100, 710, f"Engineer: {project.engineer.get_full_name()}")
    p.drawString(100, 690, f"Completed On: {project.date_of_completion.strftime('%Y-%m-%d')}")
    p.showPage()
    p.save()
    ...
    response['Content-Disposition'] = 'attachment; filename="completion_certificate.pdf"'
    return response
